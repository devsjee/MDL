...
def OpSeg(corpus,unigram_freq,DL,text):
	...
	for j in range(k,-1,-1):
		...
		dlgain = DLG(corpus,unigram_freq,DL,ngram)
	        t_score = score[j-1] + lexical_quality(ngram) + dlgain
		#lexgain = lex_stored[j-1] + lex_stored[j-1]
	    else:
		dlgain = DLG(corpus,unigram_freq,DL,ngram)
		t_score = lexical_quality(ngram) + dlgain
...
#		    lex_stored[k] = lexgain
                    print 'OS[{}] is now assigned {} with dlgain {} and score {}'.format(k,OS[k],DLG_stored[k],score[k])
